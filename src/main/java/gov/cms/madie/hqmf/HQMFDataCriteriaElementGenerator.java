...
import gov.cms.madie.hqmf.qdm_5_6.HQMFDataCriteriaGenerator;
...
public class HQMFDataCriteriaElementGenerator implements Generator {
  ...
  public void generateAttributeTagForFunctionalOp(
      MeasureExport measureExport, Node qdmNode, Element excerptElement, Node attributeQDMNode)
    ...
    createDataCriteriaForAttributes(
        ...
        measureExport.getSimpleXmlProcessor(),
        attributeQDMNode,
        true);
  }
  ...
  private void createDataCriteriaForQDMELements(
    ...
    String xPathForOccurQDMNoAttribs =
        "/measure/elementLookUp/qdm[@datatype != 'attribute'][@instance]";
    ...
    try {
      ...
      NodeList occurQdmNoAttributeNodeList =
          simpleXmlprocessor.findNodeList(
              simpleXmlprocessor.getOriginalDoc(), xPathForOccurQDMNoAttribs);
      generateOccurrenceQDMEntries(
          simpleXmlprocessor, dataCriteriaXMLProcessor, occurQdmNoAttributeNodeList);
      ...
      NodeList qdmAttributeNodeList =
          simpleXmlprocessor.findNodeList(
              simpleXmlprocessor.getOriginalDoc(), xPathForQDMAttributes);
      generateQDMAttributeEntries(
          dataCriteriaXMLProcessor, simpleXmlprocessor, qdmAttributeNodeList);
      // generating QDM Entries for default Supplemental Data Elements
      NodeList supplementalQDMNodeList =
          simpleXmlprocessor.findNodeList(
              simpleXmlprocessor.getOriginalDoc(), xpathForSupplementalQDMs);
      generateSupplementalDataQDMEntries(
          simpleXmlprocessor, dataCriteriaXMLProcessor, supplementalQDMNodeList);
    ...
    for (int j = 0; j < qdmNodeList.getLength(); j++) {
      ...
      if (qmdEntryIDNode == null) {
        createXmlForDataCriteria(
            qdmNode, dataCriteriaXMLProcessor, me.getSimpleXmlProcessor(), null);
      }
    }
  }
   ...
   * @throws XPathExpressionException the x path expression exception
   */
  private void generateOccurrenceQDMEntries(
      XmlProcessor simpleXmlprocessor,
      XmlProcessor dataCriteriaXMLProcessor,
      NodeList occurQdmNoAttributeNodeList)
      throws XPathExpressionException {
    ...
    for (Node occurNode : map.values()) {
      ...
      String xpath =
          "/measure/elementLookUp/qdm[@datatype != 'attribute'][not(@instance)][@oid = '"
              + oid
              + "'][@datatype = \""
              + datatype
              + "\"]";
   ...
   * @throws XPathExpressionException the x path expression exception
   */
  private void generateQDMEntries(
      XmlProcessor dataCriteriaXMLProcessor,
      XmlProcessor simpleXmlprocessor,
      NodeList qdmNoAttributeNodeList)
      throws XPathExpressionException {
  ...
  private void generateQDMEntry(
      XmlProcessor dataCriteriaXMLProcessor, XmlProcessor simpleXmlprocessor, Node qdmNode)
      throws XPathExpressionException {
  ...
  private void generateQDMAttributeEntries(
    ...
    if (qdmAttributeNodeList != null) {
      for (int i = 0; i < qdmAttributeNodeList.getLength(); i++) {
        ...
        generateNegationRationaleEntries(
            dataCriteriaXMLProcessor, simpleXmlprocessor, attributeQDMNode, qdmUUID);

        // Generate entries for "Value Set" attributes
        generateValueSetAttribEntries(
            dataCriteriaXMLProcessor, simpleXmlprocessor, attributeQDMNode, qdmUUID, "Value Set");
      }
    }
  ...
  private void generateNegationRationaleEntries(
    ...
    String xPathForAttributeUse =
        "/measure/subTreeLookUp/subTree//elementRef/attribute[@qdmUUID='"
            + qdmUUID
  ...
  /**
   * This method will look for attributes of mode = 'Check if Present', 'Equal To', 'Less Than (or
  ...
  private void generateDateTimeAttributeEntries(
    ...
    String xPathForAttributeUse =
        "/measure/subTreeLookUp/subTree//elementRef/attribute"
            + "[@name = '"
            + START_DATETIME
            + "' or @name='"
            + STOP_DATETIME
   ...
   * do the following,
   *
   * <p>1) Call prepForAGE_AT(MeasureExport me) which will, Look for <functionalOp type="AGE
   * AT"...../> and replace it with <relationalOp type="SBS" ..../> The first child of this new
   * relationalOp will be the elementRef for Birthdate QDM element. The 2nd child will be the first
   * child of the original <functionalOp type="AGE AT"...../>.
   */
   ...
   *
   * <p>Look for <functionalOp type="AGE AT"...../> and replace it with <relationalOp type="SBS"
   * ..../> The first child of this new relationalOp will be the elementRef for Birthdate QDM
   * element. The 2nd child will be the first child of the original <functionalOp type="AGE
   * AT"...../>.
   *
   * @param me
   */
  private void prepForAGE_AT(MeasureExport me) {
    ...
    try {
      ...
      while (ageAtFuncNode != null) {
        ...
        birthDateElementRef.setAttribute(
            "id", birthDateQDM.getAttributes().getNamedItem("uuid").getNodeValue());
        birthDateElementRef.setAttribute("type", "qdm");
        birthDateElementRef.setAttribute(
            "displayName", "Birthdate : Patient Characteristic Birthdate");
  ...
  /**
   * This method will be called by prepHQMF(MeasureExport me) method to prep functionalOp for
   * SATISFIES ALL/SATISFIES ANY functions. This methiod will do the following,
   *
   * <p>This method will look for all functionalOp's with names, SATISFIES ALL/SATISFIES ANY renames
   * the <functionalOp> tag with <setOp> tag.
   *
   * @param me
   */
