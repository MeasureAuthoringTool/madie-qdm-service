...
import gov.cms.madie.hqmf.HQMFAttributeGenerator;
import gov.cms.madie.hqmf.MatConstants;
...
public class HQMFClauseLogicGenerator implements Generator {
  ...
  private static final Map<String, String> FUNCTIONAL_OPS_NON_SUBSET =
      new TreeMap<>(String.CASE_INSENSITIVE_ORDER);

  /** MAP of Functional Ops Subset Type. */
  private static final Map<String, String> FUNCTIONAL_OPS_SUBSET =
      new TreeMap<>(String.CASE_INSENSITIVE_ORDER);
  ...
  private static final Map<String, List<String>> FUNCTIONAL_OP_RULES_IN_MO =
      new TreeMap<>(String.CASE_INSENSITIVE_ORDER);
   ...
   * referencing this child clause using <outBoundRelationShip> directly, we are adding it to a
   * default UNION grouper.
   *
   * <p>If it isnt generated then we generate it and then add a criteriaRef to it inside a default
   * UNION.
   *
  ...
  /**
   * This method is used to create a <templateId> tag for SATISFIES ALL/SATISFIES ANY functionalOps.
  ...
  /**
   * This method will basically create a <criteriaReference> with classCode='GROUPER' and
   * moodCode='EVN' and have the <id> tag pointing to the <grouperCriteria> for the referenced
   * subTree/clause.
   *
   * @param outboundRelElem the outbound rel elem
   * @param subTreeRefNode the sub tree ref node
  ...
  private List<String> checkUnUsedSubTreeRef(
      XmlProcessor xmlProcessor, List<String> usedSubTreeRefIds) {
